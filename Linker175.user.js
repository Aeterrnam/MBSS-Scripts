...
(async function () {
    ...
    while (true) {
        await sleep(100);
        // const chatEmailSelector = ".vac-text-ellipsis"; // селектор почты/телефона
       const chatEmailSelector = ".el-card__body .vac-text-last .vac-text-ellipsis";
        const chatProjectGroupSelector = ".detail-value"; // селектор названия проекта в чатах
        const archiveProjectGroupSelector = ".detail-value"; // селектор названия проекта в архивах
        // const messagesListSelector = ".css-1hdsu9g" // селектор блока, в котором чат
        const chatBgSelector = ".vac-info-wrapper"; // селектор верхнего блока, который будет подсвечен
...
// let skolkovas=document.querySelectorAll(chatEmailSelector);
        let chatEmailElem = document.querySelector(chatEmailSelector);
              // let chatEmailElem = document.querySelector(`[data-v-e2df29a4="vac-text-ellipsis"]`);
// let vseproekti=document.querySelectorAll(projectGroupSelector)[3];  // селектор именно нащвания
        let projectNameElem = document.querySelectorAll(projectGroupSelector)[3];
        if (chatEmailElem && projectNameElem && isEmailLinksEnabled) {
            ...
            // let emailmassiv = chatEmailElem.innerText.split(' ');
          // emailmassiv=emailmassiv.split(/\r\n|\r|\n/g);
           let email = chatEmailElem.innerText
           console.log(chatEmailElem)
            let emailLink;
