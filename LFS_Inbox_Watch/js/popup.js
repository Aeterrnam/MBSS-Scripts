...
zimbra_notifier_popup.initializeTooltipMessage = async function () {
    ...
    if (unreadMessages.length === 0) {
        ...
        for (index = 0; index < unreadMessages.length && currentDisplayed < nbDisplayed; index++) {
            currentDisplayed++;
            var $item = $('<div/>', { class: 'mail-list-item', tabindex: 0 });
            // SVG иконка письма
            var $icon = $('<span class="mail-list-icon" aria-label="Иконка письма"/>').html('<svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="#b80000" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><rect x="3" y="5" width="18" height="14" rx="2"/><polyline points="3 7 12 13 21 7"/></svg>');
            $item.append($icon);
            var $contentWrap = $('<div style="flex:1"></div>');
            $contentWrap.append($('<div/>', {
                class: 'mail-list-date',
                text: unreadMessages[index].date.toLocaleString() + unreadMessages[index].alias,
            }));
            $contentWrap.append($('<div/>', {
                class: 'mail-list-content',
            ...
            $item.attr('data-account-id', unreadMessages[index].controller.accountId);
            $item.on('click', function () {
                var accountId = $(this).attr('data-account-id');
                zimbra_notifier_popup
                    .driver('openZimbraWebInterface', accountId)
                    .then(() => {
                        window.close();
                    });
            });
            $('#zimbra_mail_notifier_tooltipMessage').append($item);
        }
    }
};
/**
